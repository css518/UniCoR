...
# Language label mapping dictionary
LABEL: Dict[str, int] = {
        'python': 0,
        'java': 1,
        'ruby': 2,
...
# Following the Same Operation as UnixCoder
# Special tokens for Ruby programming language
ruby_special_token: List[str] = ['keyword', 'identifier', 'separators', 'simple_symbol', 'constant', 'instance_variable',
...
# Special tokens for Java programming language
java_special_token: List[str] = ['keyword', 'identifier', 'type_identifier',  'separators', 'operator', 'decimal_integer_literal',
...
# Special tokens for Go programming language
go_special_token: List[str] = ['keyword', 'identifier', 'separators', 'type_identifier', 'int_literal', 'operator', 
'field_identifier', 'package_identifier', 'comment',  'escape_sequence', 'raw_string_literal',
'rune_literal', 'label_name', 'float_literal']

# Special tokens for JavaScript programming language
javascript_special_token: List[str] =['keyword', 'separators', 'identifier', 'property_identifier', 'operator', 
...
# Special tokens for PHP programming language
php_special_token: List[str] =['text', 'php_tag', 'name', 'operator', 'keyword', 'string', 'integer', 'separators', 'comment', 
'escape_sequence', 'ERROR',  'boolean', 'namespace', 'class', 'extends']

# Special tokens for Python programming language
python_special_token: List[str] =['keyword', 'identifier', 'separators', 'operator', '"', 'integer', 
'comment', 'none', 'escape_sequence']

# Dictionary mapping programming languages to their special tokens
special_token: Dict[str, List[str]] = {
...
# Combined list of all special tokens from all languages
all_special_token: List[str] = []
...
class InputFeatures(object):
    ...
    Attributes:
        code_ids: Tokenized code sequence IDs
        query_ids: Tokenized query sequence IDs
        comment_ids: Tokenized comment sequence IDs
        index: Index identifier
        label: Label of the example
        language: Language label converted from the input language string
    """
    def __init__(self,
                 code_ids: List[int],
                 query_ids: List[int],
    ...
    def __repr__(self) -> str:
        """Return string representation of the InputFeatures instance."""
        return str(self.to_json_string())      

def convert_examples_to_features(js: Dict[str, Any], tokenizer: Any, args: Any) -> InputFeatures:
...
def replace_with_type_tokens(inputs, replaces, speical_token_ids, tokenizer, mlm_probability):
    ...
    # 80% of the time, we replace masked input tokens with replacement tokens
    indices_replaced = torch.bernoulli(torch.full(labels.shape, 0.8)).bool().to(inputs.device) & masked_indices
...
def train(args: argparse.Namespace, 
          ...
          tokenizer: Union[RobertaTokenizer, T5Tokenizer], 
          pool: multiprocessing.Pool) -> None:
    """Train the UniCoR model using contrastive learning with optional regularization losses.
    
    Args:
        args: Command line arguments containing training configuration
        model: UniCoR model to train
    ...
    # Best model and metric tracking
    ...
    # Multiple transforms for code augmentation
    ...
    while True:
        # random sample a languages pair dataloader
...
def do_evaluate(args: argparse.Namespace, 
               model: torch.nn.Module, 
               dataloader: DataLoader) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """Evaluate the model by extracting code and natural language embeddings.
    
    Args:
        args: Command line arguments containing evaluation configuration
        model: Trained UniCoR model to evaluate
        dataloader: DataLoader for evaluation data
    
    Returns:
        Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]: Code embeddings, natural language embeddings, URLs, and labels
    """
...
def do_metric(query_vecs: np.ndarray, 
              key_vecs: np.ndarray, 
              query_uids: np.ndarray, 
              key_uids: np.ndarray, 
              query_idx: np.ndarray, 
              key_idx: np.ndarray, 
              prefix: str) -> Dict[str, float]:
    """Calculate evaluation metrics (MAP and MRR) for the model's retrieval performance.
    
    Args:
        query_vecs: Query embeddings (code or natural language)
        key_vecs: Key embeddings (code or natural language)
        query_uids: Unique identifiers for queries
        key_uids: Unique identifiers for keys
        query_idx: Indices of queries
        key_idx: Indices of keys
        prefix: Prefix for metric names in the result dictionary
    
    Returns:
        Dict[str, float]: Dictionary containing MAP and MRR scores
    """
...
def evaluate(args: argparse.Namespace, 
             ...
             eval_when_training: bool = False) -> Dict[str, float]:
    ...
    Args:
        ...
        file_name: Path to the evaluation data file
        pool: Optional pool for parallel processing
        eval_when_training: Whether evaluation is being run during training
...
def main() -> None:
    """Main function to run the training and evaluation process for the UniCoR model.
    
    This function handles argument parsing, device configuration, model initialization,
    distributed training setup, and starting the training loop.
    """
    args = parse_args()
